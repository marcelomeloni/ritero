...
export function PurchaseSection({ cafe }: PurchaseSectionProps) {
  ...
  const [pesoSelecionado, setPesoSelecionado] = useState<"250g" | "1kg">("250g");
  ...
  const currentPrice = pesoSelecionado === "250g" ? cafe.preco : cafe.preco * 3.5;

  const handleAddToCart = () => {
    addToCart({
      id: `${cafe.slug}-${pesoSelecionado}`,
      slug: cafe.slug,
      nome: cafe.produtor,
      notas: cafe.notas,
      preco: currentPrice,
          ...
          {(["250g", "1kg"] as const).map((peso) => {
            ...
            return (
              <button
                ...
                className="relative flex items-center justify-center rounded-[8px] px-6 py-3 font-mono text-[12px] font-bold tracking-[0.1em] uppercase transition-all duration-300"
                style={{
